...
class LogisticUnit:
    ...
    def __init__(self, loss_function, accuracy_function, activation_function, size, learning_rate=0.001):
        '''
    ...
    def predict(self, x):
        ...
        return  self.activation_function(np.sum((self.theta * x) + self.bias, axis=1))
    ...
    def train(self, x, y, x_validation=None, y_validation=None, ephocs=30):
        ...
        for ephoc in range(ephocs): 
            ...
            loss = np.sum(self.loss_function(y, y_prime)) #Predict again post update to get always most updates y_prime 
            ...
            if (x_validation is not None and x_validation is not None):
                ...
                validation_loss = np.sum(self.loss_function(y_validation, y_prime_validation))
